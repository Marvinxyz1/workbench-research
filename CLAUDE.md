...
生成されるファイルは**上書きしない**。毎回新しいバージョン番号を付けて保存する：

```
KPMG_Workbench_Schedule_v1.xlsx
KPMG_Workbench_Schedule_v2.xlsx
KPMG_Workbench_Schedule_v3.xlsx
```

スクリプトは `generated_docs/` フォルダ内の既存ファイルを確認し、次のバージョン番号を自動的に決定する。

### Excel 日程表のフォーマット

日程表 Excel（`generate_schedule_excel.py`）は以下のルールに従う：

1. **Phase 列**: 左端に Phase（大ブロック）列を追加
2. **セル結合**: 同じ Phase の行は縦にセル結合する
3. **背景色**: Phase 列には以下の背景色を使用
   - Phase 2（12～1月）: KPMG Blue (#005EB8)
   - 過渡期（POC討論）: 紫色 (#7030A0)
   - Phase 3（2～3月）: 緑色 (#70AD47)
   - Phase 4（4～6月）: オレンジ (#ED7D31)
4. **チーム色**:
   - KC 組: 青色 (#4472C4)
   - ATH 組: 緑色 (#70AD47)
   - 連合: 紫色 (#7030A0)
